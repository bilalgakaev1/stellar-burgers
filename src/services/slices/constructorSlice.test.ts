...
describe('constructorSlice', () => {
  ...
  it('addIngredient — заменяет булку при добавлении новой', () => {
    const stateWithBun = constructorReducer(initialState, addIngredient(mockIngredient));
    const newBun: TIngredient = { ...mockIngredient, _id: '99', name: 'Новая булка' };
  ...
  it('addIngredient — добавляет начинку', () => {
    const state = constructorReducer(initialState, addIngredient(mockIngredientMain));
  ...
  it('addIngredient — добавляет уникальный id каждому ингредиенту', () => {
    let state = constructorReducer(initialState, addIngredient(mockIngredientMain));
  ...
  it('moveIngredient — перемещает ингредиент вниз', () => {
    let state = constructorReducer(initialState, addIngredient(mockIngredientMain));
    const second: TIngredient = { ...mockIngredientMain, _id: '3', name: 'Второй' };
