...
const mockFeeds = {
    success: true,
    orders: [{ _id: '1', name: 'Test Order', number: 12345, status: 'done', ingredients: [], createdAt: '', updatedAt: '' }],
    total: 100,
    totalToday: 10,
}
...
describe('feedSlice', () => {
    ...
    it('should handle fetchFeeds.fulfilled', () => {
        const state = feedReducer(initialState, fetchFeeds.fulfilled(mockFeeds, '')); 
    ...
    it('should handle fetchFeeds.rejected', () => {
        ...
        const state = feedReducer(initialState, fetchFeeds.rejected(new Error(error), '')); 
        expect(state.isLoading).toBe(false);
