...
describe('ingredientsSlice', () => {
    ...
    it('должен вернуть начальное состояние', () => {
        expect(ingredienyReducer(undefined, { type: 'unknown' })).toEqual(initialState);
    });
    ...
    it('fetchIngredients.fulfilled — устанавливает ингредиенты и isLoading в false', () => {
        const state = ingredienyReducer({ ...initialState, isLoading: true }, fetchIngredients.fulfilled(mockIngredients, '', undefined));
    ...
    it('fetchIngredients.rejected — устанавливает ошибку и isLoading в false', () => {
        ...
        const state = ingredienyReducer({ ...initialState, isLoading: true }, fetchIngredients.rejected(new Error(errorMessage), '', undefined));
        expect(state.isLoading).toBe(false);
